...
void AlbumManager::removeUser()
{
	...
	std::list<Album> albums = m_dataAccess.getAlbums(); // get all albums for deleting removed user's albums
	std::list<Picture> picturesInCurrentAlbum; // will save pictures of current album in the loop below to untag removed user

	for (auto& album: albums) // run on the albums list
	{
		if (album.getOwnerId() == userId) // if owner id of current album is equal to removed user id
		{
			m_dataAccess.deleteAlbum(album.getName(), userId); // delte this album
		}

		else // if the owner is not the removed album, the next loop will check if there is people when he is tagged
		{
			picturesInCurrentAlbum = album.getPictures(); // get the pictures from current album
			for (auto& picture: picturesInCurrentAlbum) // run on the pictures
			{
				if (picture.isUserTagged(userId)) // if there is removed user tagged
				{
					m_dataAccess.untagUserInPicture(album.getName(), picture.getName(), userId); // untag him
				}
			}
		}
	}
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Amount of user's owned albums: " << m_dataAccess.countAlbumsOwnedOfUser(user);
}
